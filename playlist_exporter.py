...
def lst2dir(lst,dst): 
    """the 1st variable is playlist name and directory
    the other is directory of destination"""
    lst=raw(str(lst))
    dst=raw(str(dst))
    n=lst.split(".")[-1]
    if n=="xspf":
...
def raw(text):
    """this function is not mine i 've found it on the internet"""
    ...
    escape_dict={'\a':r'\a',
           ...
           '\r':r'\r',           '\t':r'\t',
           '\v':r'\v',           '\'':r'\'',       '\"':r'\"','\0':r'\000',
'\1':r'\001','\2':r'\002','\3':r'\003','\4':r'\004','\5':r'\005','\6':r'\006'}
    new_string=''
    for char in text:
...
lst=raw_input("please enter your playlist name and directory")
dst=raw_input("please enter the directory of destination folder")
lst2dir(lst,dst)
